...
export const profileUpdateValidationSchema = z
  .object({
    firstName: z
      .string()
    ...
    lastName: z
      ...
      .max(40, "Last name can't exceed 40 characters"),
...
export const requestValidationSchema = z.object({
  ...
  status: z
    ...
    .refine(
      (val) => ["ignored", "interested", "accepted", "rejected"].includes(val),
      {
        message: "Invalid status added from status",
      }
    ),
});
